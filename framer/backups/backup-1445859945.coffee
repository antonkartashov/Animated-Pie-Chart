...
# $ = Snap '#svg'; shadow = $.paper.filter(Snap.filter.shadow(0, 2, 2, "#886622", 0.2))
$ = Snap '#svg1' 
...
piechart = (animatic, sector) ->
	...
	if radian <= Math.PI
		sector.animate {
			d: "M320,320  L#{x1},#{y1}  A320,320 0 0,1 #{x2},#{y2}"
			
		}, 0
	else 
		sector.animate {
			d: "M320,320  L#{x1},#{y1}  A320,320 0 0,1 320,640  A320,320 0 0,1 #{x2},#{y2}"
		}, 0
